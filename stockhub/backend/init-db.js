...
console.log('🚀 Initializing StockHub database...\n');
...
INSERT OR IGNORE INTO categories (id, name, name_en, slug, description, level, sort_order) VALUES
(1, '电子元器件', 'Electronic Components', 'electronic-components', '各类电子元器件', 1, 1),
(2, '集成电路', 'Integrated Circuits', 'integrated-circuits', 'IC芯片、处理器等', 2, 1),
(3, '被动元件', 'Passive Components', 'passive-components', '电阻、电容、电感等', 2, 2),
(4, '连接器', 'Connectors', 'connectors', '各类连接器和插头', 2, 3),
(5, '工业设备', 'Industrial Equipment', 'industrial-equipment', '工业生产设备', 1, 2),
(6, '自动化设备', 'Automation Equipment', 'automation-equipment', '自动化生产线设备', 2, 1),
(7, '机械设备', 'Machinery Equipment', 'machinery-equipment', '各类机械加工设备', 2, 2);
...
db.exec(sql, (err) => {
  ...
  // Verify tables
  db.all("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite%'", (err, tables) => {
    if (err) {
    ...
    // Verify data
    db.all("SELECT COUNT(*) as count FROM categories", (err, result) => {
      if (err) {
        console.error('Error checking categories:', err);
      } else {
        console.log(`\n📦 Default categories: ${result[0].count} rows`);
      }
